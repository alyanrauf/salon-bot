...
// CORS middleware (add alongside existing /api/chat CORS block):
app.use(['/api/gemini-key', '/api/voice-tool'], (req, res, next) => {
  const allowed = process.env.WIDGET_ALLOWED_ORIGINS || '*';
  res.setHeader('Access-Control-Allow-Origin', allowed);
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type');
...
// ─── New voice routes (paste before app.listen) ──────────────────────────

// ─── Voice Call: expose Gemini key to widget ──────────────────────────────────
// The widget fetches this once on load; the key never sits in widget.js source.
app.get('/api/gemini-key', (req, res) => {
  const key = process.env.GEMINI_API_KEY || '';
  if (!key) return res.status(503).json({ error: 'Voice calls not configured' });
  res.json({ key });
});

// ─── Voice Call: server-side tool executor ────────────────────────────────────
// Gemini Live runs in the browser; tool calls are proxied here so they can
// read/write the SQLite DB and never expose raw data to the client.
app.post('/api/voice-tool', async (req, res) => {
  ...
  try {
    ...
    if (name === 'get_services') {
      const cur = (db.prepare("SELECT value FROM app_settings WHERE key='currency'").get() || {}).value || '';
      const rows = db.prepare('SELECT name, price, description, branch FROM services ORDER BY branch, name').all();
      result = { currency: cur, services: rows };

    } else if (name === 'get_deals') {
      const rows = db.prepare("SELECT title, description FROM deals WHERE active=1 ORDER BY id").all();
      result = { deals: rows };

    } else if (name === 'get_branches') {
      const rows = db.prepare('SELECT name, address, phone, map_link FROM branches ORDER BY id').all();
      result = { branches: rows };

    } else if (name === 'get_timings') {
      const rows = db.prepare('SELECT day_type, open_time, close_time FROM salon_timings').all();
      result = { timings: rows };

    } else if (name === 'create_booking') {
      const { customer_name, phone, service, branch, date, time, notes } = args;
      const missing = ['customer_name', 'phone', 'service', 'branch', 'date', 'time']
        .filter(function (f) { return !args[f] || !String(args[f]).trim(); });
      if (missing.length) {
        result = { ok: false, error: 'Missing required fields: ' + missing.join(', ') };
      } else {
        // Validate against salon hours
        const dow = new Date(date).getDay();
        const dayType = (dow === 0 || dow === 6) ? 'weekend' : 'workday';
        const timing = db.prepare('SELECT * FROM salon_timings WHERE day_type = ?').get(dayType);
        if (timing) {
          ...
          if ((bh * 60 + bm) < (oh * 60 + om) || (bh * 60 + bm) >= (ch * 60 + cm)) {
            result = { ok: false, error: "Outside salon hours. Open " + timing.open_time + "–" + timing.close_time + " on " + dayType + "s." };
          }
        }
        if (!result) {
          const info = db.prepare(
            "INSERT INTO bookings (customer_name, phone, service, branch, date, time, notes, status, source, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', 'voice', datetime('now'))"
          ).run(customer_name.trim(), phone.trim(), service.trim(), branch.trim(), date.trim(), time.trim(), (notes || '').trim());
          result = { ok: true, booking_id: info.lastInsertRowid, message: 'Booking confirmed!' };
        }
      }

    } else if (name === 'find_bookings') {
      const nm = (args.customer_name || '').trim();
      if (!nm) { result = { ok: false, error: 'customer_name required' }; }
      else {
        const rows = db.prepare(
          "SELECT id, service, branch, date, time, status FROM bookings WHERE customer_name LIKE ? ORDER BY date, time"
        ).all('%' + nm + '%');
        result = { ok: true, bookings: rows };
      }
    ...
    logger.error('[voice-tool] Error:', err.message);
